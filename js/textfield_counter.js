  ...
  function textWatcher(settings) {
    $.each(settings.textfieldCounter, function (key) {
      ...
      $.each(fieldSettings.key, function (index) {
        $("." + fieldSettings.key[index]).not(".description").not("div").once("textfield-counter-text-watcher").once("textfield-counter-counter-watcher").each(function () {
    ...
    if (window.hasOwnProperty("CKEDITOR")) {
      ...
      CKEDITOR.on('instanceReady', function () {
        ...
        $.each(settings.textfieldCounter ,function (fieldDefinitionKey, fieldSettings) {
          ...
          var fieldID = $("." + fieldDefinitionKey ).not('div').siblings('.cke').attr("id");
          if (fieldID) {
            fieldID = fieldID.replace("cke_", "");
